...
class TestRecipes:
    ...
    def test_recipes_pagination_and_fields(self, client, user_client, test_user):
        ...
        assert len(response_data['results']) == self.PAGE_SIZE, (
            f'{self.ENDPOINT} must return {self.PAGE_SIZE} number of instances')
        ...
        assert isinstance(tags, list), (
            f'{self.ENDPOINT} must return tags as nested object(list)'
        )
        assert isinstance(author, dict), (
            f'{self.ENDPOINT} must return author as nested object(dict)'
        )
        assert isinstance(ingredients, list), (
            f'{self.ENDPOINT} must return ingredients as nested object(list)'
        )
    ...
    def test_add_recipe_to_favorites_or_shopping_cart(self, client, user_client, test_user):
        ...
        response = user_client.get(f'{self.ENDPOINT}{recipe.id}/favorite/')
        assert response.status_code == 201
        assert FavouriteRecipe.objects.filter(user=test_user).count() == num_fav_objects + 1, (
            f'POST with correct data to {self.ENDPOINT}{recipe.id}/favorite/ must create new favoriteRecipe instance'
        )

        # Test is_favorited flag
        response = user_client.get(f'{self.ENDPOINT}{recipe.id}/')
        ...
        assert favorite_status is True, (
            f'Check {self.ENDPOINT}{recipe.id}/favorite/ adds recipe to favorites'
        )
        assert shopping_cart_status is False, (
            f'Check {self.ENDPOINT}{recipe.id}/favorite/ doesnt change is_shopping_cart flag'
        )

        response = user_client.get(f'{self.ENDPOINT}{recipe2.id}/')
        favorite_status = response.json().get('is_favorited')
        assert favorite_status is False, (
            f'Check {self.ENDPOINT}{recipe.id}/ with recipe not added to favorites shows False flag'
        )

        # Test is_in_shopping_cart flag
        response = user_client.get(f'{self.ENDPOINT}{recipe.id}/shopping_cart/')
        assert response.status_code == 201, (
            f'Check {self.ENDPOINT}{recipe.id}/shopping_cart/ adds recipe to shopping cart '
        )
        assert FavouriteRecipe.objects.filter(user=test_user).count() == num_fav_objects + 1, (
            f'{self.ENDPOINT}{recipe.id}/favorite/ must not create new favoriteRecipe instance'
        )
        response = user_client.get(f'{self.ENDPOINT}{recipe.id}/')
        shopping_cart_status = response.json().get('is_in_shopping_cart')
        assert shopping_cart_status is True, (
            f'Check {self.ENDPOINT}{recipe.id}/shopping_cart/ changes is_shopping_cart flag'
        )

        # Test delete from favorites
        response = user_client.delete(f'{self.ENDPOINT}{recipe.id}/favorite/')
        assert response.status_code == 204
        assert FavouriteRecipe.objects.filter(user=test_user).count() == num_fav_objects + 1, (
            f'{self.ENDPOINT}{recipe.id}/favorite/ shouldnt delete favRecipe instance with is_shopping_cart == True'
        )
        response = user_client.get(f'{self.ENDPOINT}{recipe.id}/')
        favorite_status = response.json().get('is_favorited')
        assert favorite_status is False, (
            f'DELETE to {self.ENDPOINT}{recipe.id}/ changes favorite flag'
        )

        # Test delete from shopping_cart
        response = user_client.delete(f'{self.ENDPOINT}{recipe.id}/shopping_cart/')
        assert response.status_code == 204
        assert FavouriteRecipe.objects.filter(user=test_user).count() == 0, (
            f'{self.ENDPOINT}{recipe.id}/shopping_cart/ must delete favRecipe instance when status flags == False'
        )
        response = user_client.get(f'{self.ENDPOINT}{recipe.id}/')
        shopping_cart_status = response.json().get('is_in_shopping_cart')
        assert shopping_cart_status is False, (
            f'DELETE to {self.ENDPOINT}{recipe.id}/ changes shopping_cart flag'
        )
    ...
    def test_delete_recipe(self, client, user_client, user2_client, test_user, test_user2):
        ...
        response = client.delete(f'{self.ENDPOINT}{recipe_by_user2.id}/')
        assert response.status_code == 401, (
            f'{self.ENDPOINT}{recipe_by_user2.id}/ must return 401 for unauthorized clients'
        )

        response = user_client.delete(f'{self.ENDPOINT}{recipe_by_user2.id}/')
        assert Recipe.objects.count() == all_recipes
        assert response.status_code == 403, (
            f'{self.ENDPOINT}{recipe_by_user2.id}/ - user cant delete recipe by another user'
        )

        response = user_client.delete(f'{self.ENDPOINT}{recipe_by_user.id}/')
        assert response.status_code == 204, (
            f'DELETE to {self.ENDPOINT} supposed to return 204'
        )
        assert Recipe.objects.filter(author=test_user).count() == num_recipes_by_user - 1, (
            f'DELETE to {self.ENDPOINT} must delete recipe instance'
        )
